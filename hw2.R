...
cv_test <- crossvalidate(cv_fmt,perceptron,r=1)

p1_train <- perceptron(y=train$X1,x=select(train,-X1,-row),r=1,E=10,seed=1234)
...
# test_fit %>% group_by(X1,fit01) %>% summarise(n=n()) %>% ungroup() %>% mutate(t=sum(n),p=n/t) %>% dcast(X1 ~ fit01,value.var='p')
